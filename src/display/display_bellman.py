...
def json_data_format(seconds: int) -> str:
    ...
    if hours > 0:
        return f"{hours} heure{plural(hours)} {minutes} minute{plural(minutes)} et {seconds} seconde{plural(seconds)}"

    return f"{minutes} minute{plural(minutes)} et {seconds} seconde{plural(seconds)}"


def json_guidage(vertices_dict: list[dict[str, Union[str, int]]],
                 bellman_ford_find_path_result: List[tuple[int, Vertex]]) -> List[dict]:
    result = []
    for current, previous, next in triowise(bellman_ford_find_path_result):
        if previous is None or current[1].ligne_metro != previous[1].ligne_metro:
            result.append({
                ...
                'start_duration_formatted': json_data_format(current[0]),
                'end_duration_seconds': next[0] if next is not None else current[0],
                'end_duration_formatted': json_data_format(next[0] if next is not None else current[0]),
        ...
        result[-1]['stations'].append({
            ...
            'correspondances': [v['ligne'] for v in vertices_dict if
                                v['name'] == current[1].nom and v['id'] != current[1].num],
        })

        result[-1]['end_duration_seconds'] = current[0]
        result[-1]['end_duration_formatted'] = json_data_format(current[0])
    ...
    for current, previous, next in triowise(bellman_ford_find_path_result):
        ...
        if previous is None:
            ...
            if previous[1].ligne_metro != current[1].ligne_metro:
                print(
                    f"{formatted_duration} - Je change de la ligne {previous[1].ligne_metro} à {current[1].nom} pour prendre la ligne {next[1].ligne_metro}")
            else:
                print(
                    f"{formatted_duration} - Je continue sur la ligne {current[1].ligne_metro} pour aller à {current[1].nom}")
